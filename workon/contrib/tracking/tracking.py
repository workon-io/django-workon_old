...
from .signals import tracking_event_created
...
def _create_create_tracking_event(instance):
    ...
    tracking_event_created.send(sender=instance.__class__, instance=instance, event=event, action=CREATE)
    ...
    for field in instance._tracked_fields:
        if not isinstance(instance._meta.get_field(field), ManyToManyField):
            try:
                ...
                # Can't compare old and new value, should be different.
                _create_tracked_field(event, instance, field)
    tracking_event_created.send(sender=instance.__class__, instance=instance, event=event, action=UPDATE)
...
def _create_delete_tracking_event(instance):
    ...
    """
    event = _create_event(instance, DELETE)
    tracking_event_created.send(sender=instance.__class__, instance=instance, event=event, action=DELETE)
    ...
    if field in getattr(model, '_tracked_related_fields', {}).keys():
        ...
        for related_field in related_fields:
            ...
            for related_instance in related_instances:
                ...
                tracking_event_created.send(sender=instance.__class__, instance=instance, event=event, action=action)
