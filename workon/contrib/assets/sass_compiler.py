...
class Watcher(object):
    ...
    def __init__(self, config=None, *args, **kwargs):
        self.config = config
        self.sass_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'sass'))
        self.cache_path = config.cache_path
        self.lock_path = f'{self.cache_path}/sass_compiler.lock'
    ...
    def process(self):
        ...
        for output, components in collections.OrderedDict(config.STYLES).items():
            ...
            file_path = f'{self.cache_path}/{file_name}.scss'
            sass_file = open(file_path, 'w+b')
            sass = f'''@charset "UTF-8";'''
            for c in components:

                if c == "workon":
                    path = f'{self.sass_path}'
                    variables_path = f'{self.cache_path}/_{c}_variables.scss'
                    variables_file = open(variables_path, 'w+b')
                    variables = str(open(f'{path}/components/_variables.scss', 'r+b').read(), 'utf-8')
                    ...
                    line = line.replace('components/variables', f'{self.cache_path}/{c}_variables')
                    line = line.replace('elements/', f'{path}/elements/')
                    line = line.replace('components/', f'{path}/components/')
                    sass += line
                    sass += f'\n@import "{self.sass_path}/forms";'
                    sass += f'\n@import "{self.sass_path}/contrib";'
