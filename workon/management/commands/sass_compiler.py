...
from contrib.utils import get_project_root

class Command(BaseCommand):
    args = ''
    help = 'ex: ./manage.py sass_compiler'

    def handle(self, *args, **options):

        materialize_path

        'sass -r sass-globbing --watch --load-path ./app ./app/sass/:./app/assets/css/ --style compressed'
        ...
        if not hasattr(settings, 'STYLUS_WATCHER'):
            ...
            for config in configs:
                try:
                    ...
                    # print config
                    self.print_error(u'Invalid config for stylus watcher "%s"' % e)
...
"""
            ...
            pipe = subprocess.Popen(cmd,
                ...
                shell = True)
            css = u"".join([ str(line, 'utf-8') for line in pipe.stdout.readlines() ])
            errors = u"".join([ str(line, 'utf-8') for line in pipe.stderr.readlines() ])
